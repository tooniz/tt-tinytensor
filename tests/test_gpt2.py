...
def tt_gpt2(target_arch):
    ...
    gamma_ln1_expand = gamma_ln1.expand(x.size()) # I think we need to make this the same size as input for tt_functional
    ...
    logging.info(f'LN MAFE: {mean_absolute_fraction_error(x_norm, tt_x_norm_cpu):.3f}')

    '''
    Q, K, V projections
    '''
    Wq, Wv, Wk = (torch.randn(1, dm, dm) for _ in range(3))
    q = torch.matmul(x_norm, Wq) # Ignore bias and weight tranposing
    ...
    gamma_ln2_expand = gamma_ln2.expand(x.size()) # I think we need to make this the same size as input for tt_functional
    ...
    print('Residual Norm MAFE:', mean_absolute_fraction_error(attn_res_norm, tt_attn_res_norm_cpu))
    ...
    print('gpt2 decoder out MAFE:', mean_absolute_fraction_error(gpt2_out, tt_gpt2_out_cpu))
